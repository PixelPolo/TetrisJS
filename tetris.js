...
import {I, O, T, L, J, Z, S} from "./tetrominos.js";
...
function goOn() {
    if (!currentTetrominos.goOneDown()) {
        ...
        if (!currentTetrominos.goOneDown()) gameOver = true;
...
document.addEventListener("keydown", (e) => {
    ...
    // ROTATIONS
    let rotation;
    if (e.key === "ArrowDown" || e.key === "s") rotation = 90;
    if (rotation) {
        currentTetrominos.rotate(rotation);
        repaint();
    }
